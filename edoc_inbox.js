...
import { file, tsImportEqualsDeclaration, ifStatement } from "@babel/types";

// var db;
//var realm;
class Edoc_Inbox extends Component {
    ...
    componentWillMount() {
        ...
        .config({trusty:true,})
        .fetch('GET','https://edoc.wim-bms.com/mobilews.asmx/Received?id=' + r.id )
        .then(async(resp)=>{
    ...
    populateupdateinbox(){
        if (this.state.selecteditem.IsOpen != 'Y') {
            let users = this.state.realm.objects('users')
            this.setState({ users: users })
            let r = users.slice(0, 1)[0]
            this.setState({ isloading: true })
            let response = RNFetchBlob
                .config({ trusty: true, })
                .fetch('GET', 'https://edoc.wim-bms.com/mobilews.asmx/OpenProcess?guid=' + this.state.selecteditem.GUID + '&iduser=' + r.id)
                .then(async (resp) => {
                    var ret = "[" + resp.data.split("[")[1].replace("</string>", "")
                    var obj = JSON.parse(ret);
                })
    ...
    renderHeader() {
      ...
      renderStatusInbox(item) {
        if (item.IsOpen!='Y') {
            // return <Badge status="success" containerStyle={{ position: 'absolute' }}/>;
            return <Text style={{paddingLeft:10, color: 'black',fontSize:12, fontWeight: 'bold', }}>{_.toUpper(item.Judul)}</Text>;
        }
        else{
            return <Text style={{paddingLeft:10, color: '#657065',fontSize:12 }}>{_.toUpper(item.Judul)}</Text>;
        }
      }
                           ...
                           <View style={{backgroundColor:  '#72b7de', flexDirection:'column',justifyContent:'center',alignItems:'flex-start'}}>
                            ...
                            <View style={{paddingLeft:10,backgroundColor:  'white', flexDirection:'row',justifyContent:'flex-start',alignItems:'flex-start'}}>
                                <TouchableOpacity  onPress={() =>  {
                                    this.setState({selecteditem:item, isModalMenu:true})} }  >
    ...
    render() {
        ...
        <View style={styles.container}>
            ...
            <Header
                ...
                centerComponent={{ text: 'Inbox', style: { color: 'white',fontSize:25 } }}
